...
모든 어댑터는 `@retemper/types`의 `ToolAdapter` 인터페이스를 구현합니다:
...
| 어댑터                                          | 패키지                          | 관리 파일            | 설명                                 |
| ----------------------------------------------- | ------------------------------- | -------------------- | ------------------------------------ |
| [adapter-eslint](/ko/adapters/eslint)           | `@retemper/adapter-eslint`      | `eslint.config.js`   | Node API를 통한 ESLint와 브릿지 파일 |
| [adapter-prettier](/ko/adapters/prettier)       | `@retemper/adapter-prettier`    | `.prettierrc`        | CLI를 통한 Prettier                  |
| [adapter-biome](/ko/adapters/biome)             | `@retemper/adapter-biome`       | `biome.json`         | 임시 설정을 사용하는 CLI 기반 Biome  |
| [adapter-husky](/ko/adapters/husky)             | `@retemper/adapter-husky`       | `.husky/<hook>`      | Husky를 통한 Git 훅                  |
| [adapter-lint-staged](/ko/adapters/lint-staged) | `@retemper/adapter-lint-staged` | `.lintstagedrc.json` | 스테이징된 파일 린팅                 |
| [adapter-commitlint](/ko/adapters/commitlint)   | `@retemper/adapter-commitlint`  | `.commitlintrc.json` | 커밋 메시지 컨벤션                   |
| [adapter-knip](/ko/adapters/knip)               | `@retemper/adapter-knip`        | `knip.json`          | 미사용 export/의존성 감지            |
| [adapter-stylelint](/ko/adapters/stylelint)     | `@retemper/adapter-stylelint`   | `.stylelintrc.json`  | CSS/SCSS 린팅                        |
