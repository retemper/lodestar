...
lodestar/
├── packages/
│   ├── types/              # @retemper/types — Shared types and interfaces
│   ├── config/             # @retemper/config — Configuration loading and normalization
│   ├── core/               # @retemper/core — Core rule execution engine
│   ├── cli/                # @retemper/cli — CLI interface
│   ├── accel/              # @retemper/accel — Native acceleration facade
│   ├── accel-typescript/   # @retemper/accel-typescript — Acceleration shared types
│   ├── accel-rust/         # @retemper/accel-rust — Rust backend (napi-rs)
│   ├── accel-go/           # @retemper/accel-go — Go backend (WASM)
│   └── lodestar/           # lodestar — Unified facade package
├── plugins/
│   ├── structure/          # @retemper/plugin-structure — Directory structure validation
│   ├── boundary/           # @retemper/plugin-boundary — Module boundary enforcement
│   ├── deps/               # @retemper/plugin-deps — Dependency graph rules
│   └── content/            # @retemper/plugin-content — Source content conventions
├── examples/
...
pnpm turbo build --filter=@retemper/core

# Run tests for a single package
pnpm turbo test --filter=@retemper/core
...
**Scope** is the package name without the `@retemper/` prefix (e.g., `core`, `cli`, `plugin-structure`).
...
# Single package
pnpm turbo test --filter=@retemper/plugin-structure
...
# With coverage
pnpm turbo test --filter=@retemper/plugin-structure -- --coverage
```
